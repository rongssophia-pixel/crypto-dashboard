...
@pytest.mark.asyncio
async def test_start_stream(ingestion_service, clean_db):
...
@pytest.mark.asyncio
async def test_stop_stream(ingestion_service, clean_db):
    ...
    # Stop it
    stop_result = await ingestion_service.stop_stream(stream_id, "test-tenant-1")
...
@pytest.mark.asyncio
async def test_get_stream_status(ingestion_service, clean_db):
    ...
    # Get status
    status = await ingestion_service.get_stream_status(stream_id, "test-tenant-1")
    ...
    # Cleanup
    await ingestion_service.stop_stream(stream_id, "test-tenant-1")


@pytest.mark.asyncio
async def test_multiple_concurrent_streams(ingestion_service, clean_db):
...
@pytest.mark.asyncio
async def test_fetch_historical_data(ingestion_service):
...
@pytest.mark.asyncio
async def test_stream_receives_data(ingestion_service, clean_db):
...
@pytest.mark.asyncio
async def test_stop_nonexistent_stream(ingestion_service, clean_db):
    """Test stopping a stream that doesn't exist"""
    result = await ingestion_service.stop_stream("non-existent-id", "test-tenant-1")
...
@pytest.mark.asyncio
async def test_tenant_isolation(ingestion_service, clean_db):
    ...
    status = await ingestion_service.get_stream_status(stream_id, "tenant-2")

    # Should fail or return error
    assert "error" in status or status.get("tenant_id") != "tenant-2"
    print("✓ Tenant isolation working correctly")

    # Cleanup with correct tenant
    await ingestion_service.stop_stream(stream_id, "tenant-1")
